...
Write-Host "1km 콘텐츠 마켓플레이스를 최신 상태로 갱신합니다..." -ForegroundColor Cyan
codex plugin marketplace upgrade $MarketplaceName

if ($LASTEXITCODE -ne 0) {
    throw "마켓플레이스 갱신에 실패했습니다. 먼저 install-windows.ps1을 실행해 주세요."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "플러그인 업데이트에 실패했습니다. 위 오류 메시지를 확인해 주세요."
}

Write-Host "업데이트가 완료되었습니다. Codex를 다시 시작하거나 새 작업을 열어 주세요." -ForegroundColor Green
